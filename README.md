...
*   **版本演进 (Changelog)**:
    *   **v4.1.22 (2026-02-20)**:
        -   **[核心重构] 统一模型配置与硬编码选项清理 (PR #2040)**:
            -   **配置统一**: 重新设计了模型配置逻辑，实现了后端模型定义的统一管理。
            -   **清理冗余**: 移除了多处硬编码的模型选项，极大提升了代码的可维护性和扩展性。
        -   **[核心优化] 优化模型配置逻辑与反代处理 (PR #2044)**:
            -   **逻辑对齐**: 进一步优化了统一后的模型配置逻辑，确保不同协议下的模型映射更加精准。
            -   **稳定性增强**: 修复了合并过程中发现的边缘情况，提升了反代服务的整体健壮性。
    *   **v4.1.21 (2026-02-17)**:
