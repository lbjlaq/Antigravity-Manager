...
- Reduce transient 429 failures by honoring upstream-provided retry delays (`RetryInfo.retryDelay` / `quotaResetDelay`) with short backoff (long delays trigger account rotation).
...
- 减少短时 429 报错：在重试前遵循上游返回的重试等待时间（`RetryInfo.retryDelay` / `quotaResetDelay`）并做短暂退避（延迟过长时改为轮换账号）。
