...
pub fn transform_claude_request_in(
    ...
    let request_id =
        format!("agent/antigravity/{}/{}", &session_id[..session_id.len().min(8)], message_count);
...
fn should_enable_thinking_by_default(model: &str) -> bool {
    ...
    {
        tracing::debug!("[Thinking-Mode] Auto-enabling thinking for Gemini Pro model: {}", model);
        return true;
    }
    ...
    if model_lower.contains("gemini-3-flash") || model_lower.contains("gemini-3.1-flash") {
        tracing::debug!("[Thinking-Mode] Auto-enabling thinking for Flash model: {}", model);
        return true;
    }
...
fn build_tools(
    ...
    if let Some(tools_list) = tools {
        ...
        if !function_declarations.is_empty() {
            ...
            func_obj.insert("functionDeclarations".to_string(), json!(function_declarations));
...
fn build_generation_config(
    ...
    if is_thinking_enabled {
        ...
        let budget_tokens =
            claude_req.thinking.as_ref().and_then(|t| t.budget_tokens).unwrap_or_else(|| {
        ...
        let global_mode_is_adaptive =
            matches!(tb_config.mode, crate::proxy::config::ThinkingBudgetMode::Adaptive);
        ...
        if should_use_adaptive {
            ...
            if lower_mapped.contains("claude") {
                ...
                tracing::debug!(
                    "[Claude-Request] Mapping adaptive mode to safe budget (24576) for Gemini model (thinkingLevel not supported)"
                );
    ...
    let global_adaptive =
        matches!(tb_config_chk.mode, crate::proxy::config::ThinkingBudgetMode::Adaptive);
